...
fn parse_exponent_no_decimal(s: &str, j: usize) -> Result<PreciseNumber, ParseNumberError> {
    ...
    // |i64::MIN| > |i64::MAX| as |−2^63| > |2^63−1|. As a result, a valid
    // negative value could cause an overflow during inversion. Let's use the
    // bounds to avoid this.
    let exponent = exponent.clamp(i64::MIN + 1, i64::MAX);
